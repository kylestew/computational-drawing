...
function variablyThickLine(
  ...
  const rotation = clockwise ? Math.PI : -Math.PI;
  const thetas = linspace(0, 1, density, true).map((t) => {
    return pctBetweenMoments(t) * rotation;
  });
...
/*
 * A DRIFTING DASH POLYLINE (pg 38)
 * ???
 *
 * PARAMS:
 * - points: list of x, y values
 * - dash: number referring to length of the dash segment
 * - gap: number referring to length of gap between dashes
 * - driftTime: amount of time recorded in steps that the segments are allowed to drift
 * - trace: true or false value that indicates whether segments should be copied as they move
 *
 * RETURNS:
 * - array of hash marks (x,y pairs forming a straight line)
 */
function driftingDashPolyline(points, dash, gap, driftTime, trace = false) {}

export { hashMark, squiggle, variablyThickLine };
